// ============================================================
// Lighthouse Reckoning — Send with ACK example (Raspberry Pi Pico + SX1262)
// Sends a test payload every 10s and tracks per-hop confirmation.
// ============================================================
...
void loop() {
    lhr_update_result_t result = lhr.update();

    switch (result) {
        case LHR_UPDATE_TX_DATA_CONFIRMED:
            // The next hop acknowledged our packet.
            // NOTE: this confirms delivery to the next relay, not to Home.
            Serial.println("[ACK] Hop confirmed.");
            break;

        case LHR_UPDATE_TX_DATA_FAILED:
            // All retry attempts exhausted — packet dropped.
            Serial.println("[ACK] Delivery failed after all retries.");
            break;
    ...
    if (millis() - lastSend >= SEND_INTERVAL_MS && !lhr.isBusy()) {
        lastSend = millis();
        ...
        lhr_err_t err = lhr.sendData(payload, sizeof(payload));
